...
export default function SubscribersScreen() {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ["subscribers"] }),
    onError: (e: any) => notify("Erro ao excluir", e?.message),
  });
  const confirmRemove = async (s: Sub) => {
    if (await confirmAsync("Excluir assinante", `Excluir ${s.name || s.email}? A conta, o operador vinculado e as sessões serão removidos. Esta ação não pode ser desfeita.`)) remove.mutate(s.user_id);
  };
  ...
  return (
    <Screen>
      <Header title="Assinantes" subtitle={`${list.length} contas · ${paid} ativas · ${list.length - paid} sem acesso`} back />
      <Row gap={8} style={{ padding: 16 }}>
        ...
        <Card style={{ flex: 1 }}>
          <Text style={styles.label}>VITALÍCIAS</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={subs.isLoading ? null : <Empty icon="people-outline" title="Nenhum assinante" hint="As contas criadas pelo app aparecerão aqui com o status de pagamento." />}
        renderItem={({ item: s }) => (
          <Card style={{ gap: 6 }} testID={`sub-${s.user_id}`}>
            <Row>
              ...
              <Badge text={s.blocked ? "bloqueado" : s.has_access ? "ativo" : "sem pagamento"} tone={s.blocked ? "error" : s.has_access ? "success" : "warning"} />
            </Row>
            ...
            <Row gap={8}>
              ...
              {!s.has_access || s.blocked ? <Button small title="Aprovar 30 dias" variant="secondary" icon="checkmark-circle-outline" onPress={() => grant.mutate({ id: s.user_id, plan: "monthly", days: 30 })} testID={`grant-${s.user_id}`} /> : null}
              <Button small title={s.blocked ? "Desbloquear" : "Bloquear"} variant={s.blocked ? "secondary" : "danger"} onPress={() => setAccess.mutate({ id: s.user_id, blocked: !s.blocked })} testID={`toggle-${s.user_id}`} />
