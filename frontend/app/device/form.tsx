...
export default function DeviceForm() {
  ...
  const ensurePermission = async (camera: boolean) => {
    ...
    if (!perm.granted) {
      if (await confirmAsync(tr("Permissão necessária"), `Permita o acesso à ${camera ? tr("câmera") : tr("galeria")} para adicionar a foto do dispositivo. Abrir configurações?`)) Linking.openSettings();
      return false;
    }
    return true;
  };
  ...
  const doSave = async () => {
    if (!d.name.trim()) {
      setErr(tr("Informe um nome/identificação"));
      return;
    }
    setErr("");
    try {
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }} keyboardShouldPersistTaps="handled">
          <Row gap={12}>
            ...
            <View style={{ flex: 1, gap: 6 }}>
              <Text style={styles.hint}>{tr("Foto do dispositivo no local")}</Text>
              <Button small title={tr("Tirar foto")} icon="camera-outline" variant="secondary" onPress={() => takePhoto(true)} loading={uploading} testID="device-camera" />
              <Button small title={tr("Galeria")} icon="image-outline" variant="ghost" onPress={() => takePhoto(false)} testID="device-gallery" />
            </View>
          </Row>

          <Text style={styles.section}>{tr("TIPO DE DISPOSITIVO")}</Text>
            ...
            {DEVICE_KINDS.map((k) => {
              ...
              return (
                <Pressable key={k.value} testID={`device-kind-${k.value}`} onPress={() => setD((p) => ({ ...p, kind: k.value }))} style={[styles.kind, active && styles.kindActive]}>
                  ...
                  <Text style={[styles.kindText, active && { color: colors.onBrandTertiary }]}>{tr(k.label)}</Text>
                </Pressable>
              );
            })}
          </View>

          <Text style={styles.section}>{tr("IDENTIFICAÇÃO")}</Text>
          <Input label={tr("Nome / identificação *")} value={d.name} onChangeText={set("name")} placeholder="PC Recepção 01" error={err} testID="device-name" />
          <Row gap={8}>
            <Input style={{ flex: 1 }} label={tr("Patrimônio")} value={d.asset_tag} onChangeText={set("asset_tag")} placeholder="PAT-0001" autoCapitalize="characters" testID="device-tag" />
            <Input style={{ flex: 1 }} label={tr("Nº de série")} value={d.serial} onChangeText={set("serial")} placeholder={tr("Se houver")} autoCapitalize="characters" testID="device-serial" />
          </Row>
          <Row gap={8}>
            <Input style={{ flex: 1 }} label={tr("Marca")} value={d.brand} onChangeText={set("brand")} placeholder="Dell" testID="device-brand" />
            <Input style={{ flex: 1 }} label={tr("Modelo")} value={d.model} onChangeText={set("model")} placeholder="OptiPlex 3090" testID="device-model" />
          </Row>
          <Input label={tr("Setor / local onde está alocado")} value={d.sector} onChangeText={set("sector")} placeholder={tr("Recepção, Financeiro, CPD...")} testID="device-sector" />
          <Select label={tr("Cliente")} value={d.client_id ?? ""} options={(clients.data ?? []).map((c) => ({ value: c.id, label: c.name }))} onChange={(v) => setD((p) => ({ ...p, client_id: v, client_name: clients.data?.find((c) => c.id === v)?.name ?? "" }))} testID="device-client" />

          <Text style={styles.section}>CONFIGURAÇÃO · {kind.label.toUpperCase()}</Text>
          <Text style={styles.hint}>{tr("Preencha somente o que souber; campos em branco são permitidos.")}</Text>
          {kind.specs.map((f) => (
            <Input key={f.key} label={tr(f.label)} value={d.specs[f.key] ?? ""} onChangeText={setSpec(f.key)} placeholder={f.placeholder} testID={`spec-${f.key}`} />
          ))}

          <Text style={styles.section}>{tr("MANUTENÇÃO")}</Text>
          <Select label={tr("Periodicidade da preventiva")} value={String(d.preventive_months)} options={PREVENTIVE_OPTIONS} onChange={(v) => setD((p) => ({ ...p, preventive_months: Number(v) }))} testID="device-preventive" />
          <Select label={tr("Status")} value={d.status} options={DEVICE_STATUS} onChange={(v) => setD((p) => ({ ...p, status: v }))} testID="device-status" />
          <Input label={tr("Observações")} value={d.notes} onChangeText={set("notes")} multiline />
        </ScrollView>
      </KeyboardAvoidingView>
      <StickyBar>
        <Button title={tr("Salvar dispositivo")} icon="save-outline" onPress={doSave} loading={save.isPending} style={{ flex: 1 }} testID="device-save" />
      </StickyBar>
    </Screen>
  );
}
